...
public class EventServiceImpl extends ServiceImpl<EventMapper, Event> implements IEventService {
        ...
        List<EventDTO> eventDTOs = eventPage.getRecords().stream()
                .map(event -> {
                    ...
                    switch (event.getEventType()) {
                        case 0:
                            dto.setEventType("confirm");
                            break;
                        case 1:
                            dto.setEventType("false_alarm");
                            break;
                        case 2:
                            dto.setEventType("emergency");
                            break;
                        default:
    ...
    @Override
    public Result saveEvent(String description, String response, Integer eventType) {
        UserDTO user = UserHolder.getUser();
        if (user == null) {
            return Result.fail("用户未登录");
        }
